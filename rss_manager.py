            ...
            try:
                ...
                for row in csv_reader:
                    if len(row) >= 1:
                        ...
                        try:
                            title, description, last_build_date = (
                                RSSFeed.fetch_feed_info(url)
                            )
                        except Exception as e:
                            errors.append(
                                f"Error fetching info for RSS Feed {url}: {str(e)}"
                            )
                        ...
                        for feed in feeds_to_add:
                            try:
                                ...
                                logging_config.logger.info(
                                    f"RSS Feed added from CSV: {feed.url}"
                                )
                            except IntegrityError:
                                db.session.rollback()
                                skipped_count += 1
                                logging_config.logger.info(
                                    f"Skipped duplicate RSS Feed: {feed.url}"
                                )
                        if imported_count > 0:
                            flash(
                                f"Successfully imported {imported_count} RSS feeds.",
                                "success",
                            )
                        if skipped_count > 0:
                            flash(
                                f"Skipped {skipped_count} duplicate RSS feeds.", "info"
                            )
