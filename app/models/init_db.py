...
from models.relational.allgroups import (
    Base as AllGroupsBase,
    AllGroups,
    AllGroupsValues,
    AllGroupsValuesNames,
)
from sqlalchemy import create_engine, text, inspect
from app.services import apt_update_service as update_databases_module
import json
import logging
    ...
    with app.app_context():
        ...
        try:
            ...
            required_tables = [
                ...
                "alltools",
                "alltools_values",
                "alltools_values_names",
                "allgroups",
                "allgroups_values",
                "allgroups_values_names",
            ]
        ...
        with engine.connect() as connection:
            result = connection.execute(text("PRAGMA table_info(parsed_content)"))
            columns = {row[1]: row[2] for row in result.fetchall()}
            if "summary" not in columns:
                connection.execute(
                    text("ALTER TABLE parsed_content ADD COLUMN summary TEXT")
                )
            if "art_hash" not in columns:
                connection.execute(
                    text("ALTER TABLE parsed_content ADD COLUMN art_hash VARCHAR(64)")
                )
            elif columns["art_hash"] != "VARCHAR(64)":
                connection.execute(
                    text("ALTER TABLE parsed_content DROP COLUMN art_hash")
                )
                connection.execute(
                    text("ALTER TABLE parsed_content ADD COLUMN art_hash VARCHAR(64)")
                )
            ...
            result = connection.execute(text("PRAGMA table_info(rss_feed)"))
            columns = {row[1]: row[2] for row in result.fetchall()}
            if "awesome_blog_id" not in columns:
                connection.execute(
                    text("ALTER TABLE rss_feed ADD COLUMN awesome_blog_id VARCHAR(36)")
                )

            # Add missing columns to allgroups_values if they don't exist
            result = connection.execute(text("PRAGMA table_info(allgroups_values)"))
            columns = {row[1]: row[2] for row in result.fetchall()}
            missing_columns = {
                "motivation": "TEXT",
                "first_seen": "TEXT",
            ...
            for column, column_type in missing_columns.items():
                if column not in columns:
                    connection.execute(
                        text(
                            f"ALTER TABLE allgroups_values ADD COLUMN {column} {column_type}"
                        )
                    )
        ...
        print(
            f"All database tables created and updated successfully in {app.instance_path}"
        )
        ...
        try:
            ...
            logging.info(
                f"JSON data (first 1000 characters): {json.dumps(json_data)[:1000]}"
            )
            ...
            logging.error(
                f"JSON snippet around error: {e.doc[max(0, e.pos-50):e.pos+50]}"
            )
